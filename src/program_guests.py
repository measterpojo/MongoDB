...
def add_a_snake():
    ...
    name = input('What must log in first to add a snake')
    if not name:
        error_msg('cancelled')
        return
...
def view_your_snakes():
    ...
    snakes = svc.get_bookings_for_user(state.active_account.id)
...
def book_a_cage():
    print(' ****************** Book a cage **************** ')
    if not state.active_account:
        error_msg('You muust lod in first to book a cage')
        return
    
    snakes = svc.get_snakes_for_user(state.active_account.id)
    if not snakes:
        error_msg('You must first [a]dd a snake before you can bok a cage')
        return
    
    print("Let's start by finding available cages.")
    start_text = input('Chech-in date [yyyy-mm-dd]:')
    if not start_text:
        error_msg('cancelled')
        return
    ...
    snake = snakes[int(input('WHich snkae do you want to book')) - 1]
    ...
    cage=cages[int(input('WHich cage do oyu want to book (number)')) -1]
